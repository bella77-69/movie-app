...
const commingSoonFile = path.join(__dirname, '../data/commingSoon.json');

function listSoon() {
    return JSON.parse(fs.readFileSync('./data/commingSoon.json', 'utf-8'));
}
...
function addList(body) {
    ...
    fs.writeFileSync(commingSoonFile, JSON.stringify(listArr));
    return list;
}
...
router.get('/:id', (req, res) => {
    ...
    fs.readFile('./data/commingSoon.json', 'utf-8', (err, data) => {
        if(err) {
            console.log(err);
            res.json({message: 'error getting movie id data'});
        }
        const listData = JSON.parse(data);
        const foundList = listData.find((data) => data.id == req.params.id);
        if(!foundList) {
            res.json({message: 'error getting movie data'});
        } else {
            res.json(foundList)
        }
    });
